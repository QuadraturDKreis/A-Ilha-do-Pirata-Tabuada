...
export const GAME_NODES: Record<number, GameNode> = {
  ...
  8: {
    ...
    onEnter: (s) => (s.history.filter(n => n === 8).length === 1 ? { ...s, time: s.time - 10 } : s)
  },
  9: {
    id: 9,
  ...
  14: {
    ...
    onEnter: (s) => {
      ...
      if (s.time <= 60) newState.events.avisoMareBaixa = true;
      return newState;
    }
  },
  15: {
  ...
  20: {
    ...
    onEnter: (s) => {
      ...
      if (s.history.filter(n => n === 20).length === 1) newState.time -= 10;
      return newState;
    }
  },
  21: {
  ...
  32: {
    ...
    onEnter: (s) => {
      ...
      if (s.history.filter(n => n === 32).length === 1) newState.time -= 10;
      if (newState.time <= 40) {
  ...
  36: {
    ...
    onEnter: (s) => {
      if (!s.items.chaveDourada || s.time < 30) return s;
      return {
        ...s,
        time: s.time - 10,
  ...
  38: {
    ...
    onEnter: (s) => {
      let newState = { ...s };
      if (s.time <= 20) newState.events.modoUltimaChamada = true;
      return newState;
    }
  },
  39: {
  ...
  42: {
    ...
    onEnter: (s) => {
      if (s.time < 30 || s.events.modoUltimaChamada) return s;
      return { ...s, time: s.time - 10 };
    }
  },
  43: {
    id: 43,
  ...
  50: {
    ...
    onEnter: (s) => {
      ...
      if (s.time >= 30 && !s.events.rodadaRelampago) {
