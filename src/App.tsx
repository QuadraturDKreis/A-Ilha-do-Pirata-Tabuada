...
export default function App() {
  ...
  const resetGame = useCallback(() => {
    if (confirm("Deseja realmente reiniciar a aventura? Todo o progresso será perdido.")) {
  ...
  const startGame = (name: string) => {
    if (!name.trim()) return alert("Por favor, digite seu nome pirata!");
    let newState = { ...INITIAL_STATE, playerName: name, isStarted: true, history: [1] };
  ...
  const getColorByTime = (time: number) => {
    if (time > 50) return 'bg-emerald-500';
          ...
          <div className="space-y-4">
            <div className="relative">
              ...
              <input 
                ...
                className="w-full pl-12 pr-4 py-4 rounded-2xl bg-slate-50 border-2 border-slate-200 focus:border-sky-400 focus:ring-0 outline-none transition-all"
          ...
          <div className="p-8 space-y-8">
            ...
            <div>
              <h3 className="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2">
                <ShieldCheck className="text-sky-600" /> Suas Conquistas
              </h3>
            ...
            <div className="grid grid-cols-2 gap-4 text-center">
              <div className="bg-slate-100 p-4 rounded-2xl">
                ...
                <div className="text-2xl font-bold text-slate-700">{gameState.time} min</div>
              </div>
              <div className="bg-slate-100 p-4 rounded-2xl">
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col font-sans">
                ...
                {!currentNode.isErrorCorridor && currentNode.choices && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {currentNode.choices.map((choice, i) => {
                      // Logic for specific requirements in choices
                      ...
                      if (isExtraMission) {
                        if (gameState.currentNode === 14 && (gameState.time <= 50 || gameState.missions.ajudouPipo)) isDisabled = true;
                        if (gameState.currentNode === 26 && (gameState.time <= 30 || gameState.missions.ajudouBina)) isDisabled = true;
                      }
                      
                      if (isChest) {
                        if (choice.nextNode === 18 && !gameState.items.chaveAzul) isDisabled = true;
                        if (choice.nextNode === 36 && (!gameState.items.chaveDourada || gameState.time < 30)) isDisabled = true;
                        if (choice.nextNode === 42 && (gameState.time < 30 || gameState.events.modoUltimaChamada)) isDisabled = true;
                      }
                        ...
                        >
                          <div>
                            <span className={`text-[10px] font-black uppercase tracking-widest block mb-1 ${isExtraMission ? 'text-amber-500' : 'text-slate-400'}`}>
                              {isExtraMission ? '✦ Missão Extra' : isChest ? '✧ Tesouro' : `Opção ${i+1}`}
                            </span>
      ...
      <footer className="py-6 text-center text-slate-400 text-xs font-medium">
        Labirintos da Matemática — Versão Digital 5.0 | Rodrigo Mota Narcizo
      </footer>
    </div>
  );
}
